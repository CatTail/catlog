...
parser.parse_permalink = function parse_permalink (env) {
  ...
  env.permalink_style = this.permalink_styles[env.permalink_style] ||
    env.permalink_style || this.permalink_styles.date;
  debugger;
  return env.permalink = env.permalink_style.replace(/:(\w+)/g, 
                                               function (match, item) {
